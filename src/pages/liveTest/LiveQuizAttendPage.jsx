...
import { getMegaQuizAttendQuetionGetSlice, megaQuizAttendSubmitSlice } from '../../redux/LiveQuizeSlice';




const LiveQuizAttendPage = () => {
...
//   return
  ...
  const getTestSeriesQuestion = async () => {
    try {
      setLoading(true)
      const res = await dispatch(getMegaQuizAttendQuetionGetSlice(state?.testInfo?.id)).unwrap()
...
if (!current || questionsState.length === 0) {
  ...
  const handleConfirmPause = async () => {
    ...
    try {
      ...
      exitFullScreen();
      nav('/previous-papers', { replace: true, state: { testId: state?.testId } });
    ...
    <div className="flex flex-col p-4 text-sm font-sans overflow-hidden w-full">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
        ...
        <div className="w-full lg:w-auto m-auto bg-gray-800 text-white rounded-sm">
          <TestTimer textleft={'Time Left:'} testId={state?.testInfo?.id} timeInMinutes={state?.testInfo?.duration} onTimeUp={() => handleSubmit()} />
        </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-y py-4 mb-3 gap-3">
        ...
        <div className="flex flex-col lg:flex-row gap-4 w-full lg:w-1/2 items-start lg:items-center justify-between">
          ...
          <div className="text-right w-full lg:w-auto">
            <TestTimer timeClr='text-blue-800' textleft={'LAST'} textBg='text-red-600' timeTextSize='text-2xl' textRight={'Minutes'} showSeconds={false} testId={state?.testInfo?.id} timeInMinutes={state?.testInfo?.duration} onTimeUp={() => handleSubmit()} />
          </div>
        </div>
      </div>
...
export default LiveQuizAttendPage;
