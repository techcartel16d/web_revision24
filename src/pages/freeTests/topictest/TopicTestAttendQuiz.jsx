...
import {
  attendPreviouseYearQuestionSlice,
  getPreviouseYearGetQuestionSlice,
...
const TopicTestAttendQuiz = () => {
  ...
  const testId = state?.testInfo?.id;
  ...
  const groupedQuestions = questionsState.reduce((acc, question) => {
    const subject = acc.find(
      (grp) => grp.subject_name === question.subject_name
    );
    if (subject) {
  ...
  const handleSaveAndNext = async () => {
    ...
    if (isAlreadyMarkedWithAns) {
      const updatedMarkedWithAns = markedWithAns.filter(
        (id) => id !== currentId
      );
      setMarkedWithAns(updatedMarkedWithAns);
      await secureSaveTestData(testId, "marked_with_ans", updatedMarkedWithAns);
  ...
  // ✅ agar loading true hai → spinner show karo
  if (loading) {
    return (
  ...
  // ✅ agar loading khatam ho gaya aur data nahi mila → not found
  if (!current || questionsState.length === 0) {
    return (
      <div className="p-4 text-red-500 w-full h-full flex items-center justify-center">
        <p>Test not found</p>
      </div>
  ...
  const handleConfirmPause = async () => {
    ...
    const currentTestId = state?.testInfo?.id;

    try {
      // 🔁 Always use 'pause_status' as key
      const existingStatus =
        (await secureGetTestData("pause_status", "pause_status_array")) || [];

      // 🧹 Remove old entry for current test if it exists
      const updatedStatus = existingStatus.filter(
        (item) => item.test_id !== currentTestId
      );
  ...
  const questionText =
    language === "en" ? current.question_english : current.question_hindi;
  // console.log("question text", questionText)

  const totalTimeSpent = Object.values(spentTime).reduce(
    (sum, val) => sum + val,
    0
  );
  ...
  const handleSubmit = async () => {
    ...
    const optionSelected2 =
      (await secureGetTestData(testId, "optionSelected")) || [];
    const selectedOptions2 =
      (await secureGetTestData(testId, "selectedOptions")) || {};
    const skippedQuestions =
      (await secureGetTestData(testId, "skippedQuestions")) || [];
    const markedForReview =
      (await secureGetTestData(testId, "markedForReview")) || [];
    const totalAttendedQuestions = optionSelected2.length;
    const totalNotAnsweredQuestions =
      questionsState.length - totalAttendedQuestions;
    ...
    const marksScored = correct * markPer_ques - in_correct * 2;
    const totalTimeSpent = spentTime.reduce(
    ...
    try {
      ...
      if (res.status_code == 200) {
        ...
        showErrorToast(
          "please attend minimum one question (save & next) after submit!!"
        );
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4 gap-4">
        ...
        <div className="flex flex-wrap justify-between lg:justify-end items-center gap-3 w-full lg:w-auto">
          ...
          <div className="text-sm">
            Name :{" "}
            <span className="font-semibold">{userInfo.name || "guest"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-y py-4 mb-3 gap-3">
        <div className="text-red-600 font-semibold text-center flex flex-wrap gap-3 w-full lg:w-auto">
          <button
            ...
            className="text-orange-600 font-bold px-4 py-2 rounded text-base"
          >
            SYMBOLS
          </button>
          <button
