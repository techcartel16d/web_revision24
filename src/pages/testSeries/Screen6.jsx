...
const Screen6 = () => {
    ...
    // ✅ FIXED: Single, consolidated fetchUserResult function with attend_id support
    // const fetchUserResult = useCallback(async () => {
    //     if (!state) {
    ...
    //     // ✅ FIXED: Fallback to API call with BOTH test_id and attend_id
    //     let testId = state?.testInfo?.test_id ||
    //         state?.testInfo?.id ||
    //         state?.testData?.my_detail?.test_id;
    ...
    //     console.log('📡 No preloaded data, making API call with:', {
    //         test_id: testId,
    //         attend_id: attendId
    //     });
    ...
    //         // ✅ FIXED: Pass both test_id and attend_id as object
    ...
    //     }
    // }, [dispatch, state]);
    // ✅ In Screen6.jsx fetchUserResult function
    ...
    const fetchUserResult = useCallback(async () => {
        ...
        // ✅ Get test_id and attend_id from multiple sources
        let testId = state?.testInfo?.test_id ||
            state?.testInfo?.id ||
            state?.testData?.my_detail?.test_id ||
            state?.actualTestId;

        let attendId = state?.attend_id ||  // ✅ First check direct attend_id
            state?.testInfo?.attend_id ||
            state?.testData?.my_detail?.attend_id ||
            secureGetTestData('currentTest')?.attend_id;  // ✅ Fallback to storage
        ...
        try {
            ...
            // ✅ Pass both test_id and attend_id
            const res = await dispatch(fetchUserTestSeriesRankSlice({
                test_id: testId,
                attend_id: attendId
            })).unwrap();
